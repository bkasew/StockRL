...
#from stockEnv import StockTradingEnvSeq
...
class Evaluator:
    def __init__(self, cwd: str, agent_id: int, eval_env, args: Arguments):
        ...
        self.eval_times2 = int(args.eval_times2)
        ...
        print(f"{'#' * 80}\n"
              f"{'ID':<3}{'Step':>8}{'maxR':>8}{'test_return':>15} |"
              f"{'avgR':>8}{'stdR':>7}{'avgS':>7}{'stdS':>6} |"
              f"{'expR':>8}{'objC':>7}{'etc.':>7}")


    def evaluate_save_and_plot(self, act, steps: int, r_exp: float, log_tuple: tuple, config, how = "step") -> Tuple[bool, bool]:

        if how not in ["step", "time"]:## make sure 'how' options are available
            raise ValueError("Invalid sim type. Expected one of: ['step', 'time']")
        self.total_step += steps  # update total training steps
        
        if self.total_step % self.eval_steps > steps - 1: ## if the step is more than horizon_len above eval_steps
            if_reach_goal = False
            if_save = False
        
        elif (how == "time") & (time.time() - self.eval_time < self.eval_gap): ## if using time and not time
            if_reach_goal = False
            if_save = False
        
        else: ## otherwise, run the evaluator
            ...
            '''record result on test environment'''
            test_return = self.evaluate_test(act, config)
            ...
            if if_reach_goal and self.used_time is None:
                ...
                print(f"{'ID':<3}{'Step':>8}{'TargetR':>8} |"
                      ...
                      f"{self.used_time:>8}  ########")
            print(f"{self.agent_id:<3}{self.total_step:8.2e}{self.r_max:8.2f}{test_return:15.5f} |"
                  f"{r_avg:8.2f}{r_std:7.1f}{s_avg:7.0f}{s_std:6.0f} |"
    ...
    def evaluate_test(self, act, config):
        env = StockTradingEnvSeq(**config)
        ...
        for steps in range(env.max_step):
            ...
            tensor_action = act(tensor_state.to(device)).argmax(dim=1) if if_discrete else act(tensor_state) # different
            action = tensor_action.detach().cpu().flatten()  # not need detach(), because using torch.no_grad() outside
...
def get_cumulative_returns_and_step(env, act) -> Tuple[float, int]:  
    ...
    for steps in range(max_step):
        ...
        tensor_state = torch.as_tensor(state, dtype=torch.float32)#.unsqueeze(0) # -- remove unsqueeze
        tensor_action = act(tensor_state).argmax(dim=1) if if_discrete else act(tensor_state) # different
        action = tensor_action.detach().cpu().flatten()  # not need detach(), because using torch.no_grad() outside
        state, reward, done, _ = env.step(action.numpy())
        returns += reward
        if done:
