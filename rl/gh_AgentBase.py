...
#net_dim: int, state_dim: int, action_dim: int
class AgentBase:
    def __init__(self, action_dim, state_dim, actor_args: dict, critic_args: dict, gpu = False, args: Arguments = None):
        ...
        ########## HERE WE ARE ##########
        self.act = act_class(state_dim=state_dim, action_dim=action_dim, **actor_args).to(self.device)
        self.cri = cri_class(state_dim=state_dim, action_dim=action_dim, **critic_args).to(self.device) \
            if cri_class else self.act
        #################################
    ...
    def explore_one_env(self, env, target_step: int, done) -> list:
        """
        ...
        i = 0
        #done = False
        state = env.reset() if done else env.get_state()
        while i < target_step:
            tensor_state = torch.as_tensor(state, dtype=torch.float32)#.unsqueeze(0) # -- remove unsqueeze
            tensor_action = self.act.get_action(tensor_state.to(self.device)).detach().cpu().flatten()  # different
            next_state, reward, done, _ = env.step(tensor_action.numpy())  # different

            traj_list.append((tensor_state, reward, done, tensor_action))  # different

            i += 1
            ## if done, reset environment; otherwise, continue with next state
            state = env.reset() if done else next_state

        self.states[0] = state
        last_dones[0] = i
        return self.convert_trajectory(traj_list, last_dones), done  # traj_list
    ...
    ## NON VECTORIZED
    def convert_trajectory(self, traj_list: List[Tuple[Tensor, ...]], last_done: Union[Tensor, list]) -> List[Tensor]:
        # assert len(buf_items[0]) in {4, 5}
        # assert len(buf_items[0][0]) == self.env_num
        traj_list1 = list(map(list, zip(*traj_list))) ## CHANGED: use list() instead of []
        del traj_list
        ...
        traj_state = torch.cat(traj_list1[0]) #torch.stack(traj_list1[0])
        traj_action = torch.stack(traj_list1[3])
        ...
        if self.env_num > 1:
            ...
            traj_reward = (torch.tensor(np.array(traj_list1[1]), dtype=torch.float32) * self.reward_scale).reshape(-1, 1)  ## .reshape(-1, 1, 1)
            traj_mask = ((1 - torch.tensor(np.array(traj_list1[2]), dtype=torch.float32)) * self.gamma).reshape(-1, 1)  ## .reshape(-1, 1, 1)
        ...
        for j in range(len(traj_list2)):
            ...
            for env_i in range(self.env_num):
                ...
                cur_item.append(buf_item[:last_step])  ## cur_item.append(buf_item[:last_step, env_i])

                if self.if_use_old_traj:
                    self.traj_list[env_i][j] = buf_item[last_step:]  ## agent.traj_list[env_i][j] = buf_item[last_step:, env_i]

            traj_list3.append(torch.vstack(cur_item))
            # traj_list3.append(cur_item[0])
        del traj_list2
