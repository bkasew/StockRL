...
class DataProcessor:
    def __init__(self, data_source, **kwargs):
        ...
        self.tech_indicator_list = kwargs.get("tech_indicator_list", None)

    def download_data(
        self, ticker_list, start_date, end_date, time_interval, adjst="raw"
    ) -> pd.DataFrame:
        self.adjst = adjst
    ...
    def add_technical_indicator(self, df, tech_indicator_list=None) -> pd.DataFrame:
        if self.tech_indicator_list is None:
            if tech_indicator_list is None:
                raise Exception("No Technical Indicators Given")
            else:
                self.tech_indicator_list = tech_indicator_list
        df = self.processor.add_technical_indicator(df, self.tech_indicator_list)
    ...
    def add_vix(self, df) -> pd.DataFrame:
        df = self.processor.add_vix(df, self.adjst)
